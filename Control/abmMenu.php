...
class abmMenu
{
    ...
    public function armarMenu()
    {
        ...
        if ($sesion->sesionActiva()) {
            $permisos = [];
            $rol = null;
            //Primero obtengo los roles
            $roles = $sesion->getRoles();
            if (count($roles) > 0) {
                $arregloRolActual = $sesion->getRolActivo();
                //Obtenemos el rol actual para armar el menu
                $objMR = $this->ObtenerMenu(['idrol' => $arregloRolActual['id']]); 
                $rol = strtoupper($arregloRolActual['rol']);
                foreach ($objMR as $actualMR) {
                    //Listamos todos los permisos para ese rol
                    $permisoActual = $this->listarPermisos($actualMR->getObjMenu());
                    array_push($permisos, $permisoActual);
                }
            }

            //Obtenemos sus roles, si es que tiene
            $arregloRolesUser = $sesion->getRoles(); 
